...
final class AuthPlugin: PluginType {
    ...
    func prepare(_ request: URLRequest, target: TargetType) -> URLRequest {
        ...
        case .bearer:
            guard let token = AuthPlugin.bearerToken else { return request }
        ...
        case .login:
            do {
                ...
                if #available(OSX 10.12, *) {
                    ...
                    fatalError("`dateDecodingStrategy` Unavailable")
                }
                let token = try response.map(BearerToken.self, using: decoder)
                AuthPlugin.bearerToken = token.value
            } catch {
        ...
        case .logout:
            do {
                ...
                if response.statusCode == 200 {
                    AuthPlugin.bearerToken = nil
                }
            } catch {
                // Some logout must have occurred
            }
        default:
            break
        }
