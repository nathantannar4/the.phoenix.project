...
public struct APNSMessage {
    ...
    public init(priority: Priority = .immediately, profile: Profile, deviceToken: String, payload: APNSPayload, on container: Container, development: Bool = false) throws {
    ...
    internal func generateRequest(on container: Container) throws -> Request {
        ...
        request.http.body = try HTTPBody(data: encoder.encode(APNSPayloadContent(payload: self.payload)))
